...
% $Revision: 1.26 $
% $Author: js229 $
% $Date: 2013-07-24 23:13:34 +0100 (Wed, 24 Jul 2013) $
% $Log: Venn.Rnw,v $
...
\usepackage{Sweave}
\SweaveOpts{prefix.string=figdir/Venn,debug=TRUE,eps=FALSE,echo=FALSE,pdf.version=1.4}
\usepackage{float}
\usepackage{natbib}
\usepackage{mathptmx}
\usepackage{rotating} 
...
For these basic plots, use of the \texttt{Vennerable} package may sometimes be
...
The calls to \texttt{plot} are really convenience wrappers for two separate functions; the 
first computes the geometry of the drawing, returning an object of class \texttt{VennDrawing},
and the second renders that object.
For example
<<pnosho,echo=TRUE,eval=TRUE>>=
...
layout,  is not shown. However all the cose used can be seen in  
\texttt{edit(vignette("Venn"),editor="internal")}.}
...
\begin{figure}[H]\begin{center}
<<pV4uw,fig=TRUE,cache=TRUE,eval=TRUE>>=
plotV4uw <- function(V) {
	grid::grid.newpage()
	grid::pushViewport(viewport(layout=grid.layout(2,2)))
	doavp <- function(type) {
...
\begin{figure}[H]\begin{center}
<<p2no11threef,fig=TRUE,eval=TRUE>>=
...
points. 
\begin{figure}[H]\begin{center}
<<p2s01threef,fig=TRUE,eval=TRUE>>=
...
\begin{figure}[H]\begin{center}
<<pv3wempty1t,fig=TRUE,eval=TRUE>>=
...
\begin{figure}[H]\begin{center}
<<pv3wempty2t,fig=TRUE,eval=TRUE>>=
...
corresponding intersection subset must be members of the $i$-th subset. Depending on the application, the 
